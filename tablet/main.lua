...
-- Portable remote access terminal for WiRe Server.             --
-- Original WiRe created by Dog (HydrantHunter).                --
--==============================================================--

local WiReTver = "0.4.0-dev"
local configFile = "/wire/tablet.cfg"
local thisCC = os.getComputerID()
local serverId, network, serverColour, serverName
local searchText = ""
local page = 1
local perPage = 8
local deviceCache = {}
...
local function loadConfig()
  if not fs.exists(configFile) then return end
  ...
  if ok and type(cfg) == "table" then
    ...
    serverColour = cfg.serverColour
    serverName = cfg.serverName
    recent = type(cfg.recent) == "table" and cfg.recent or {}
    if serverColour then network = "WiRe" .. serverColour end
  end
end
...
local function colourValue(name)
  if not name then return colors.gray end
  local n = string.lower(tostring(name))
  if colors[n] then return colors[n] end
  return colors.gray
end

local function header(title)
  local w = term.getSize()
  term.setBackgroundColor(colourValue(serverColour))
  term.setTextColor(colors.white)
  term.setCursorPos(1, 1)
  term.write(string.rep(" ", w))
  term.setCursorPos(2, 1)
  term.write(title or "WiRe Tablet")
  term.setCursorPos(math.max(1, w - #WiReTver), 1)
  term.write(WiReTver)
  term.setBackgroundColor(colors.black)
  term.setTextColor(colors.white)
end
...
local function sendPacket(payload)
  if not serverId or not network then return false, "No server paired" end
  payload.program = payload.program or "WiReTablet"
  payload.cc = tonumber(thisCC)
  payload.color = serverColour
  payload.tabletVersion = WiReTver
  local dataPack = textutils.serialize(payload)
  local encKey = tostring(serverId) .. "WiRe!Comms" .. tostring(thisCC)
  local encryptedPackage = encode(encrypt(encKey, dataPack))
  rednet.send(serverId, encryptedPackage, network)
  return true
end

-- rednet.receive wrapper is easier and keeps compatibility with classic ComputerCraft.
local function request(payload, timeout)
  local ok, err = sendPacket(payload)
  if not ok then return nil, err end
  local id, encryptedMessage = rednet.receive(network, timeout or 3)
  if not id then return nil, "timeout" end
  if id ~= serverId or type(encryptedMessage) ~= "string" then return nil, "bad sender" end
  local okDecode, decoded = pcall(decode, encryptedMessage)
  if not okDecode or type(decoded) ~= "string" then return nil, "decode failed" end
  local encKey = tostring(thisCC) .. "WiRe!Comms" .. tostring(serverId)
  local okDec, plain = pcall(decrypt, encKey, decoded)
  if not okDec or type(plain) ~= "string" then return nil, "decrypt failed" end
  local okUn, packet = pcall(textutils.unserialize, plain)
  if not okUn or type(packet) ~= "table" then return nil, "bad packet" end
  return packet
end

local function pairSetup()
  clear()
  header("WiRe Tablet Setup")
  print()
  print("Enter WiRe server colour exactly as used during setup.")
  print("Example: Purple")
  print()
  write("Server colour: ")
  serverColour = read()
  if serverColour == "" then return false end
  network = "WiRe" .. serverColour
  if not findModem() then
    pause("No modem found.")
    return false
  end
  clear(); header("Pairing")
  print(); print("Looking for WiRe server on " .. network .. "...")
  serverId = rednet.lookup(network, serverColour)
  if not serverId then
    pause("No server found. Check colour, modem and server status.")
    return false
  end
  local info = request({ request = "SERVERINFO" }, 3)
  if info and (info.response == "SERVERINFO" or info.response == "TABLET_SERVERINFO") then
    serverName = info.serverName or ("Server " .. serverId)
  else
    serverName = "Server " .. serverId
  end
  saveConfig()
  pause("Paired with " .. serverName .. " (ID " .. tostring(serverId) .. ")")
  return true
end

local function refreshDevices()
  local packet, err = request({ request = "DEVICE_LIST", search = searchText }, 5)
  if packet and packet.devices then
    deviceCache = packet.devices
    serverName = packet.serverName or serverName
    status = "Connected"
    saveConfig()
    return true
  end
  status = "Device list failed: " .. tostring(err)
  return false
end

local function refreshGroups()
  local packet, err = request({ request = "LISTGROUPS" }, 5)
  if packet and packet.groups then
    groupCache = packet.groups
    serverName = packet.serverName or serverName
    status = "Connected"
    saveConfig()
    return true
  end
  status = "Group list failed: " .. tostring(err)
  return false
end

local function addRecent(kind, name)
  if not name then return end
  local entry = kind .. ":" .. name
  for i = #recent, 1, -1 do
    if recent[i] == entry then table.remove(recent, i) end
  end
...
local function sendDeviceCommand(deviceName, cmd)
  local packet, err = request({ request = "COMMAND_DEVICE", device = deviceName, cmd = cmd }, 5)
  if packet and packet.ok then
    addRecent("D", deviceName)
    status = "Sent " .. cmd .. " to " .. deviceName
    return true
  end
  status = "Failed: " .. tostring(err or (packet and packet.error))
  return false
end

local function runGroup(groupName)
  local packet, err = request({ request = "COMMAND_GROUP", group = groupName }, 5)
  if packet and packet.ok then
    addRecent("G", groupName)
    status = "Ran group " .. groupName
    return true
  end
  status = "Failed: " .. tostring(err or (packet and packet.error))
  return false
...
local function chooseCommand(deviceName)
  clear(); header(deviceName)
  print()
  print("1. ON / OPEN")
  print("2. OFF / CLOSE")
  print("3. TOGGLE")
  ...
  write("Command: ")
  local c = read()
  if c == "1" then sendDeviceCommand(deviceName, "ON")
  elseif c == "2" then sendDeviceCommand(deviceName, "OFF")
  elseif c == "3" then sendDeviceCommand(deviceName, "TOGGLE") end
end

local function filteredDevices()
  local list = {}
  local s = string.lower(searchText or "")
  for i = 1, #deviceCache do
    local d = deviceCache[i]
    local name = tostring(d.name or d.deviceName or d.cc or "Unknown")
    local state = tostring(d.state or d.deviceState or "")
    if s == "" or string.find(string.lower(name .. " " .. state), s, 1, true) then
      list[#list+1] = d
    end
  end
  table.sort(list, function(a,b)
    return tostring(a.name or a.deviceName or a.cc or "") < tostring(b.name or b.deviceName or b.cc or "")
  end)
  return list
end

local function devicesScreen()
  refreshDevices()
  while true do
    clear(); header(serverName or "WiRe Tablet")
    print("Devices   Search: " .. (searchText ~= "" and searchText or "<none>"))
    print("Status: " .. status)
    print(string.rep("-", term.getSize()))
    local list = filteredDevices()
    local pages = math.max(1, math.ceil(#list / perPage))
    page = math.min(math.max(1, page), pages)
    local start = ((page - 1) * perPage) + 1
    for i = 0, perPage - 1 do
      local d = list[start + i]
      if d then
        local idx = i + 1
        local name = tostring(d.name or d.deviceName or d.cc or "Unknown")
        local state = tostring(d.state or d.deviceState or "?")
        print(idx .. ". " .. name .. " [" .. state .. "]")
      end
    end
    print(string.rep("-", term.getSize()))
    print("N Next  P Prev  S Search  R Refresh  0 Back")
    write("Select: ")
    local c = string.lower(read())
    if c == "0" then return
    elseif c == "n" then page = math.min(page + 1, pages)
    elseif c == "p" then page = math.max(page - 1, 1)
    elseif c == "s" then write("Search: "); searchText = read(); page = 1; refreshDevices()
    elseif c == "r" then refreshDevices()
    else
      local n = tonumber(c)
      if n and list[start + n - 1] then
        local d = list[start + n - 1]
        chooseCommand(tostring(d.name or d.deviceName or d.cc))
      end
    end
  end
end

local function groupsScreen()
  refreshGroups()
  while true do
    clear(); header(serverName or "WiRe Tablet")
    print("Groups")
    print("Status: " .. status)
    print(string.rep("-", term.getSize()))
    for i = 1, math.min(#groupCache, perPage + 2) do
      print(i .. ". " .. tostring(groupCache[i]))
    end
    print(string.rep("-", term.getSize()))
    ...
    local c = string.lower(read())
    if c == "0" then return
    elseif c == "r" then refreshGroups()
    else
      local n = tonumber(c)
      if n and groupCache[n] then runGroup(groupCache[n]); pause(status) end
    end
  end
end

local function recentScreen()
  while true do
    clear(); header(serverName or "WiRe Tablet")
    print("Recent")
    print(string.rep("-", term.getSize()))
    for i = 1, #recent do
      local kind, name = string.match(recent[i], "^(.-):(.*)$")
      print(i .. ". " .. (kind == "G" and "Group " or "Device ") .. name)
    end
    print(string.rep("-", term.getSize()))
    print("0 Back")
    write("Select: ")
    ...
    local n = tonumber(c)
    if n and recent[n] then
      local kind, name = string.match(recent[n], "^(.-):(.*)$")
      if kind == "G" then runGroup(name); pause(status) else chooseCommand(name) end
    end
  end
end
...
local function home()
  while true do
    clear(); header(serverName or "WiRe Tablet")
    print("Connected to: " .. tostring(serverName or "Not paired"))
    print("Server ID: " .. tostring(serverId or "?"))
    print("Colour: " .. tostring(serverColour or "?"))
    print("Status: " .. status)
    print()
    print("1. Devices")
    print("2. Groups")
    print("3. Recent")
    print("4. Pair / Re-pair")
    print("0. Exit")
    print()
    write("Select: ")
    local c = read()
    if c == "1" then devicesScreen()
    elseif c == "2" then groupsScreen()
    elseif c == "3" then recentScreen()
    elseif c == "4" then pairSetup()
    elseif c == "0" then clear(); return end
  end
end

clear()
if not http then status = "HTTP not needed after install" end
if not findModem() then
  print("No modem found. Attach a wireless modem.")
  return
end
loadConfig()
if not serverId or not serverColour then pairSetup() end
home()
